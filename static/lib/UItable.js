...
function refreshObjectTable() {
    // If parameters not initialized yet, skip until setup() completes
    if (typeof _parameterList === 'undefined' || !_parameterList || typeof _parameterList.length === 'undefined') {
        const el = document.getElementById("divObjProperties");
        if (el) el.innerHTML = "";
        return;
    }
    let i, j;
